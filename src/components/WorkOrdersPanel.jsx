...
import { BriefcaseBusiness, Plus, AlertCircle } from 'lucide-react';
    ...
    return (
      <Card className="workorders-panel-state error">
        <div className="workorders-error">
          <AlertCircle size={18} />
          <span>Error cargando Work Orders</span>
        </div>
      </Card>
    );
  }
    ...
    <div className="workorders-panel">
      ...
      <div className="workorders-panel-header">
        <div className="workorders-panel-heading">
          ...
          <div>
            <h2>Work Orders</h2>
            <p>Gestión operativa y económica del proyecto</p>
          </div>
        </div>

        {/* {can(PERMISSIONS.WO_CREATE) && ( */}
          <Button onClick={() => onCreate?.()} className="create-wo-btn">
            <Plus size={16} />
            Nueva Work Order
          </Button>
        {/* )} */}
      ...
      {workOrders.length === 0 && (
        <Card className="workorders-empty-state">
          ...
          <h3>No hay Work Orders</h3>
          <p>Este proyecto todavía no tiene Work Orders asociadas.</p>
